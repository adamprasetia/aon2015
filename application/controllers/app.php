...
class App extends MY_Controller {
	...
	function _check_blank(){
		...
		$data = array('com_1','com_2','dem_1','dem_2','dem_3','dem_4','dem_5');
		foreach($data as $r){
	...
	function export(){
		...
		$active_sheet->setCellValue('AO1', 'AUDIT');
		$active_sheet->setCellValue('AP1', 'SALAH');
		$active_sheet->setCellValue('AQ1', 'User Entry');
		$active_sheet->setCellValue('AR1', 'Date Entry');
		$active_sheet->setCellValue('AS1', 'Country');
		...
		foreach($result as $r){
			...
			$active_sheet->setCellValue('AO'.$i, $r->audit);
			$active_sheet->setCellValue('AP'.$i, $r->salah);
			$active_sheet->setCellValue('AQ'.$i, $r->fullname);
			$active_sheet->setCellValue('AR'.$i, PHPExcel_Shared_Date::PHPToExcel(date_to_excel($r->date_create)));
			$active_sheet->getStyle('AR'.$i)->getNumberFormat()->setFormatCode('dd/mm/yyyy');		   
			$active_sheet->setCellValue('AS'.$i, $r->country);
			$i++;
		}
